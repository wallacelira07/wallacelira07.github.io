...
## -5. Continuação do bloco 30 — botão de print da seção "07 Livros Razão": tentativa de reposicionar QUEBROU o download, revertida

Usuário achou que o "⬇" de download JPEG do canto superior direito era algo nativo do navegador — esclarecido: é feature real do site (`inicializarBotoesPrintSecao()`, `ui-componentes-visuais.js`, pedido de sessão anterior, 03/08/2026), não algo do sistema operacional. Usuário mandou print com seta apontando de lá até um ponto logo abaixo da grade de abas, pedindo pra mover o botão pra esse ponto.

**1ª tentativa (revertida)**: mover o botão pra dentro do `.card`, logo depois de `#lrTabs`, via `insertAdjacentElement('afterend', btn)` + classe `.btn-print-secao--ancorado-abas`. **Usuário reportou quebra real**: baixando o JPEG das abas LRC (Corporativo) em diante, o arquivo vinha só com a grade de abas, sem a tabela — LRW até LRR continuavam normais. Causa: `baixarSecaoComoJPEG()`/`html2canvas` captura exatamente o elemento `.card` — o botão, que antes vivia FORA do `.card` (no cabeçalho `.section-num`, irmão do card, nunca capturado), passou a viver DENTRO dele. Mecanismo exato não confirmado (suspeita: interação entre o estado "carregando…" do próprio botão dentro da árvore capturada e a animação `v300FadeInPane` das abas carregadas via V2/async, mais lentas que as abas com dado V1 estático como LRR) — mas a correlação com a mudança era clara o bastante pra reverter sem insistir tentando diagnosticar sem acesso a navegador real.

**Revertido por completo**: botão de volta ao cabeçalho pra TODAS as seções (comportamento idêntico ao de antes de hoje). `.btn-print-secao--ancorado-abas` removida do CSS. **Pedido original de reposicionar fica pendente** — precisa de uma solução que não insira nada dentro do próprio elemento que `html2canvas` captura (ex.: um botão duplicado fora do `.card`, posicionado via `position:fixed`/`sticky` visualmente perto do fim das abas, ou mover só o CABEÇALHO inteiro pra dentro do fluxo do card com CSS) — e precisa ser validada com download real de pelo menos 2-3 abas diferentes (uma V1 estática tipo LRR, uma V2 assíncrona tipo LRC) antes de considerar resolvida, não só testada visualmente.

**Não validado em navegador real com login** (mesma limitação de sempre) — mas a REGRESSÃO foi encontrada e reportada pelo próprio usuário ao vivo, então o revert acima é a prioridade confirmada, não suposição.
