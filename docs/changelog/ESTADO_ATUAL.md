...
7. **Medidor da Wellida**: cron dedicado no cron-job.org ainda não criado (usuário tem a URL/config); `medidor_tuya_wellida` ainda não monitorado em Saúde Operacional (esperar cron rodar sozinho primeiro). Confirmado 18/08 (print do app Smart Life): `energia_total_kwh` real é mesmo 0,00 kWh até agora (bate exato com o app, não é bug de sincronização — contador ainda não passou do 1º incremento, mesmo com potência ativa ~76W). Medidor fica offline intermitentemente (WiFi) — **usuário já comprou um medidor substituto, chega domingo (23/08/2026)**; até lá, seguir usando os dados deste (fica offline de vez em quando, mas volta sozinho).
