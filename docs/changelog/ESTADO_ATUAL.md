...
### 5.3 Bug estrutural achado ao vivo: `liquidoReal` preso no salário do ciclo passado

Print real do usuário mostrou o card "Necessidade × Salário" tratando R$16.819,56 (salário recebido em 24/07, do ciclo ANTERIOR) como se fosse o líquido confirmado do ciclo atual (Ago/26), que na verdade ainda não tinha recebido salário nenhum (esperado só 25/08). Causa raiz: `REG.superavitNormal.liquidoReal = {0: 16819.56}` (`reg-operacional.js`), escrito em 25/07 quando o índice 0 realmente era aquele ciclo — nunca foi limpo depois que o ciclo virou. Já tinha causado o mesmo problema em 19/08 ("eu estava gastando como se fosse receber 16.8k"), mas aquela correção mexeu só na lógica (`liquidoMes()`), não no dado. Corrigido: `liquidoReal: {}` (vazio, estado correto até o próximo salário real ser confirmado). **Risco estrutural permanece**: precisa ser limpo manualmente a cada virada de ciclo, sem rollover automático ainda.
