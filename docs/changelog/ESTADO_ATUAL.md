...
## -5. Continuação do bloco 30 — botão de print da seção "07 Livros Razão" reposicionado
...
**Implementado, escopo restrito a essa 1 seção**: em `inicializarBotoesPrintSecao()`, se o `.card` da seção contém `#lrTabs` (id único da grade de abas do Livro Razão), o botão é ancorado logo depois da grade (`insertAdjacentElement('afterend', btn)`) em vez de no cabeçalho — classe nova `.btn-print-secao--ancorado-abas` (`assets/css/styles.css`, `display:block;margin:0.6rem 0 0 auto`) empurra pra direita sem `position:absolute`. As outras ~47 seções do site continuam com o botão no cabeçalho, sem mudança. Guarda de duplicata (`if (header.querySelector(...) || card.querySelector(...))`) ajustada pra procurar nos 2 lugares possíveis.

**Nota honesta dada ao usuário**: não tenho confirmação de que reposicionar o botão resolve o sintoma relatado ("toda vez que vou tirar print está fechando") — a hipótese mais provável continua sendo algo do navegador/SO (ferramenta de captura rolável) ou, alternativamente, `html2canvas` em `scale:2` sendo pesado o bastante nessa seção específica (25 abas + tabelas grandes) pra travar um navegador mobile com pouca memória. Reposicionamento foi feito porque foi pedido explicitamente e é de baixo risco — não descarta investigar a causa raiz de verdade se o sintoma persistir.

**Não validado em navegador real com login** (mesma limitação de sempre).
