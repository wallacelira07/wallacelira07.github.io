...
Usuário conferiu as 38 linhas do LRW-Wallace uma por uma (print + soma manual) contra o rodapé/contador — confirmado por SQL que 37 transações reais + 1 limbo (TX000132, já corrigido) = 38, somando exatamente R$2.611,83, batendo com o sistema.

**Achado real do usuário no meio da conferência**: `TX000272` ("TOKIO MARINE*AUTO", R$200,99, 06/08) é o **seguro do carro**, uma cobrança recorrente — não deveria estar na Caixa Variável como compra pontual. Conferido: o `cartao_id` dessa transação já era o Visa Infinite ativo (4845, Vanessa), não nem Mastercard Black — outro caso da mesma contaminação Visa↔MB já documentada (view sem filtro de bandeira).

**Corrigido**: nova recorrência `TXRR000007` ("Tokio Marine (Seguro Auto)", R$200,99, `ultima_cobranca_em`=06/08/2026) em `cronograma_recorrencias`. `usuario_id` de `TX000272` setado pra NULL (mesmo padrão já usado pra Digna/Pax Domini/Vivo — tira da lista LRW sem apagar a transação, ela já está representada pela recorrência). **Correção de cartão**: cadastrei inicialmente como "Visa Infinite" (seguindo o `cartao_id` real da transação, que aponta pro Visa 4845) — usuário corrigiu na hora: "é recorrência no Mastercard". Campo `cartao` da recorrência atualizado pra "Mastercard Black" (`TX000272` em si continua com `cartao_id` do Visa na base, não realinhado — só a classificação da recorrência mudou).

**Novo total esperado do LRW**: R$2.611,83 → **R$2.410,84** (37 lançamentos, menos o Tokio Marine, que sai daqui). **Efeito no "Não Reconciliado" agora é NEUTRO** (não reduz nem aumenta): o valor só migrou de bucket (LRW → Recorrências), continua contando pro MB do mesmo jeito — diferente do que eu tinha documentado antes de errado (achando que era Visa e sairia da conta de vez). **Ainda existe outra contaminação Visa conhecida e NÃO corrigida** (`TX000274`, R$113,72, Anthropic Claude Sub no Visa Infinite 4845) — essa sim parece ser Visa de verdade (não corrigida pelo usuário como a de cima), mas não mexida ainda porque pode ser referenciada por uma reconciliação do lado Visa que eu não conferi; não remover sem checar isso primeiro.

**Nota**: há um agente especialista rodando em paralelo nesta mesma investigação (reconciliação Mastercard Black) — esta seção documenta um achado feito DIRETAMENTE pelo usuário durante a espera, fora do escopo do agente. Quem ler o relatório do agente depois deve cruzar com esta seção pra não perder este achado.

## -16. Continuação do bloco 31 — Origem vazia + data sem ano nas 2 exceções do limbo (LRW/LRV)

Usuário mandou print: LRW mostrando "TX000132, 22/07 (sem ano), App de alinhamento solar (Google SunSurveyor) — limbo, Origem vazia, R$56,99" e "linha torta" (coluna Valor deslocada). Mesmo achado em LRV (TX000154, H57Store, R$30,97).

**Causa real, achada rápido**: `aplicarOnda3LrwLrvListaDetalhada()` (`hydrate-onda3-lrwlrv.js`) insere 2 objetos hand-coded na lista pra cobrir o "limbo" (compras de 22-24/07 que a view `vw_transacoes_cartao_variavel_por_pessoa` exclui por filtrar `data >= ciclo_inicio_em`, 25/07) — mas esses 2 objetos nunca tiveram o campo `cartaoId` que todo outro item da lista tem (vem de `mapear()`, sempre preenche `cartaoId: l.cartao_id || null`). Sem esse campo, `linha()` (`render-livros-variaveis.js`) não desenha a célula de Origem — a linha vinha com 1 `<td>` a menos que o cabeçalho (5 colunas), explicando tanto a Origem vazia quanto a coluna Valor "torta" (deslocada pra esquerda por faltar uma célula). Data também estava sem o ano (`'22/07'` hardcoded, não `'DD/MM/YYYY'` como os outros itens).

**Corrigido**: `cartaoId` adicionado nos 2 objetos, com o cartão real confirmado pela fatura (TX000132 = 2244, TX000154 = 6351) — data também ganhou o ano.

**Não testado ao vivo** (mesma limitação de sempre).

## -15. Continuação do bloco 31 — caixas temáticas RETOMADAS na soma do Não Reconciliado, depois de confirmar item a item que são reais

Usuário pediu a lista completa dos R$2.865,85 de caixas temáticas (12 transações, 3 caixas: Bens Duráveis R$1.638,09/9, Emagrecimento R$868,89/2, Churrasco R$358,87/1) e perguntou "todas essas compras estão na fatura, porque está dando divergência?" — pergunta certa: TODAS as 12 batem com a fatura real (confirmado item a item, incluindo os casos onde 1 linha da fatura vira 2-3 linhas em `transacoes` por causa de split entre caixas, ex. TX000159/159-A/159-B = R$551,01 da fatura dividido em Caixa Variável + Bens Duráveis, sem sobra nem falta).

**Causa real da divergência não era dado — era código**: `recalcularEHidratarMbPessoal()` (`hydrate-visa-mb.js`) calcula `D.caixasTematicas` normalmente, mas nunca somava esse valor em `somaPartes`/`naoReconciliado` — ficava só como "dado informativo" na tela, por causa de uma tentativa anterior (bloco 29-30) que piorou o resultado (+1.248 → -1.617) antes de qualquer uma das causas reais (âncora desalinhada, dupla-contagens) serem descobertas.

**Recalculado à mão com os números já corrigidos desta sessão** (âncora R$7.024,31, sem Digna/Pax Domini/Mega/Claude duplicados, recorrências com filtro de ciclo): somar `caixasTematicas` agora dá um resíduo de ~**-R$521,49** (negativo, mas MUITO mais perto de zero que o -R$1.617,62 da tentativa anterior) — perto o bastante de bater pra valer a pena testar ao vivo de novo.

**Implementado**: `somaPartes` em `recalcularEHidratarMbPessoal()` agora inclui `D.caixasTematicas` (com fallback `|| 0` pro caso do fetch async ainda não ter chegado). Linha "Caixas temáticas" continua exibida separadamente na tela (informativa), só a fórmula do Não Reconciliado mudou.

**Não testado ao vivo** — usuário deve conferir o card. Se o resíduo ficar negativo e pequeno (perto de -R$500, batendo com o esperado), a pendência está efetivamente fechada (diferença residual explicável por pequenas taxas de IOF/câmbio já documentadas, seção -10). Se vier muito diferente do esperado, investigar de novo — não assumir que está certo só porque a conta bateu no papel.

## -14. Continuação do bloco 31 — âncora estava defasada (só ia até 17/08) + confirmada cobertura completa de caixas

Usuário perguntou 2 coisas certeiras depois de ver R$1.728,03 de resíduo:

1. **"Você já acrescentou as compras de 17 até hoje?"** — não tinha. O xlsx só cobria até 17/08, mas hoje é 20/08 e o usuário já tinha mostrado (print do banco) 4 compras reais de 18-19/08 não capturadas na âncora: Medidor Wellida R$93,51 + Churrasqueira R$349,99 (18/08) + Nobre Carnes R$82,83 + MP*Brothersclub R$90,00 (19/08) = R$616,33, todas já confirmadas como corretamente lançadas no banco (achado de sessões anteriores). Mais Amazon Prime R$19,90 (assinatura, já conta via LRS mas não estava incluída na âncora). **Âncora atualizada: R$6.407,98 → R$7.044,21** (`parametros_gerais` + literal de fallback em `vars-mercado-pago.js`).

2. **"Você está somando TODAS as caixas?"** — verificado por SQL sem nenhum filtro de lista pré-definida (só cartão MB + ciclo atual, agrupado por caixa): confirmado que só 5 caixas têm gasto real este ciclo — Caixa Variável, Bens Duráveis, Emagrecimento, Churrasco, Caixa Wärtsilä (corporativo). As outras 6 das "9 caixas temáticas" do código (Combustível, Mastercard_Infinite, Manutenção, Eventos, Saúde Família, Seguro Emplacamento) têm zero gasto real no MB este ciclo — não é lacuna de cobertura, `MB_CAIXAS_TEMATICAS_IDS` já cobre todas as caixas que historicamente compram no MB. **Confirmado: cobertura de caixas está completa, não é essa a causa do resíduo restante.**

**Resíduo esperado depois da atualização da âncora**: ~R$2.364 (não testado ao vivo) — caixas temáticas (R$2.865,85) continua sendo a peça que falta encaixar, mas a diferença entre os dois números (~R$500) é bem menor que o overshoot anterior (~R$2.100), sugerindo que a defasagem da âncora era boa parte do problema, não só as caixas temáticas.

## -13. Continuação do bloco 31 — recorrências ganham consciência de ciclo real do cartão (mudança de código, não só dado)

Usuário confirmou item a item: Brisanet cobra de novo dia 25 (já ciclo seguinte), Rastreador do carro pago 20/07, próxima esperada hoje (20/08, ainda não confirmada), Faculdade só 11/09 — nenhuma das 3 recorreu de verdade dentro do ciclo atual do cartão (fecha dia 22). Anthropic/Claude: usuário esclareceu que os 6 lançamentos no MB são gasto real e pontual de 1 mês de uso pesado, depois cancelado — não uma assinatura recorrente; a assinatura "Claude" de verdade só foi paga no Visa Infinite este mês (volta pro Mastercard mês que vem). Pediu explicitamente pra implementar a mudança de código, não só ajustar dado.

**Implementado**:
1. **Migração** `add_ultima_cobranca_em_recorrencias`: nova coluna `cronograma_recorrencias.ultima_cobranca_em` (date, NULL-safe — ausente é tratado como "não cobrou ainda", nunca assume que cobrou por segurança).
2. **7 recorrências populadas** com a data real da última cobrança confirmada (Brisanet/Faculdade/Rastreador = 17-20/07, antes do ciclo atual; Digna/Campo Santo da Paz/Vivo = 25-30/07, dentro do ciclo atual).
3. **`mbCicloAtualInicio()`** (`hydrate-onda9-livros-fixos.js`): calcula a data de início do ciclo REAL do cartão (fecha sempre dia 22) a partir de hoje — se já passou do dia 22 deste mês, ciclo começou dia 22 deste mês; senão, dia 22 do mês anterior.
4. **`VARS.mbLRRConfirmado`/`visaLRRConfirmado`** agora só somam recorrências cujo `ultima_cobranca_em` cai DENTRO do ciclo atual do cartão — `ativo=true` continua controlando o que aparece na TABELA do LRR (visão de "obrigações recorrentes", sem mudança), só a soma usada no Não Reconciliado ficou consciente de ciclo.
5. **`cronograma_assinaturas` "Claude" (TXS000002) desativado** (`ativo=false`) — os 6 lançamentos reais de Anthropic no MB já contam via LRW (não são assinatura recorrente, confirmado pelo usuário), manter a linha ativa dobraria a conta.

**Resultado esperado (calculado, não testado ao vivo)**: o resíduo do "Não Reconciliado" deve SUBIR de R$762,78 pra aproximadamente **R$1.815,99** — não é regressão, é o número ficando mais HONESTO depois de remover contagens duplas reais (Digna R$152,41 + Pax Domini R$77,79 + Mega R$30,99 + Claude R$120,60 = R$381,79) e recorrências que ainda não cobraram este ciclo (R$614,45). **Caixas temáticas (R$2.865,85, ainda fora da soma) continua sendo o principal componente não explicado** — investigado a fundo nesta sessão (seção -12) sem conclusão definitiva sobre por que somar direto continua dando overshoot mesmo depois de eliminar as duplicidades encontradas.

**Não testado ao vivo** (sem acesso a login) — usuário deve conferir o card "Não Reconciliado" depois de publicar e reportar o valor real mostrado.

## -12. Continuação do bloco 31 — usuário questionou "não tem algo errado" (com razão): 2 novos bugs reais de dupla contagem achados e corrigidos, mais 1 causa estrutural identificada (não corrigida, precisa do usuário)

Depois da seção -11 (resíduo caiu de 1.201 pra 763), usuário perguntou diretamente "não tem algo errado, as transações dos LRs são as compras que fiz no cartão, como não tá batendo?" — pergunta certa, não aceitei o resíduo como "normal" e investiguei mais.

**2 bugs reais de dupla contagem achados e CORRIGIDOS** (eu mesmo tinha causado 2 deles ao religar `usuario_id` sem checar se já existiam como recorrência):
1. **TX000241 "Digna Vida" (R$152,41)** e **TX000249 "Pax Domini/Campo Santo da Paz" (R$77,79)** — são as MESMAS recorrências já cadastradas em `cronograma_recorrencias` ("Digna" e "Campo Santo da Paz"). Eu tinha religado o `usuario_id` dessas 2 transações na seção -11 sem checar isso primeiro — `usuario_id` revertido pra NULL de novo (mesmo padrão já usado corretamente pra "Vivo").
2. **TXS000010 "Mega" (R$30,99)** — já existe em `cronograma_assinaturas` ("MEGA Pro Lite"), mas a transação bruta correspondente estava com `ja_orcado_assinaturas=false` (deveria ser `true`) — contava 2x. Corrigido (`ja_orcado_assinaturas=true`).

**Causa estrutural maior, achada mas NÃO corrigida (precisa decisão do usuário, item por item)**: comparando a soma de `cronograma_assinaturas`+`cronograma_recorrencias` (R$1.910,00) contra o que a fatura real realmente mostra como cobrança de assinatura/recorrência no ciclo atual — 22/07 em diante (R$1.351,14, calculado por regex sobre os 114 itens reais) — sobra uma diferença de R$558,86, mas ela vem de 2 desvios em direções OPOSTAS que quase se cancelam:
- **Recorrências cobradas ANTES do ciclo atual, mas contadas mesmo assim**: Faculdade Engenharia (R$441,33) e provavelmente Brisanet/Rastreador do carro foram cobradas em 17/07 — ANTES do ciclo atual do cartão começar (22/07). A própria nota da Faculdade já dizia "próxima cobrança normal só em 11/09" — ou seja, ela NÃO deveria contar neste ciclo, mas `cronograma_recorrencias` soma o valor de toda recorrência `ativo=true` sem checar se ela realmente recorreu dentro da janela do ciclo atual.
- **Anthropic/Claude sub-contado**: a fatura real mostra **6 cobranças reais da Anthropic neste ciclo** (04/08 R$117,18 + 06/08 R$116,42 + 11/08 R$116,52 + 03/08 R$52,95+R$21,16+R$52,95 = **R$477,18 no total**), mas `cronograma_assinaturas` "Claude" tem só R$120,60 (a atualização da seção -10 pegou só 1 das 6 cobranças) — mesmo problema já documentado da Anthropic cobrando errado/múltiplas vezes em agosto/2026.

**Não corrigido nesta sessão** — precisa do usuário confirmar, item por item, quais recorrências realmente já foram cobradas de novo dentro do ciclo atual (22/07+) antes de eu ajustar `cronograma_recorrencias`/`cronograma_assinaturas` pra valores certos. Mexer nisso às cegas de novo é o mesmo tipo de erro que já causou o "piorou de +1.248 pra −1.617" documentado antes.

## -11. Continuação do bloco 31 — CAUSA RAIZ REAL do "Não Reconciliado" achada: incompatibilidade entre ciclo do cartão e ciclo do site, âncora corrigida

Depois de fechar o gap de R$35 (seção -10 abaixo), usuário pediu pra recalcular o "Não Reconciliado" do painel de verdade. Print ao vivo mostrou resíduo de **R$1.201,35** — bem maior que os ~R$35 da reconciliação item a item. Investigado a fundo em vez de aceitar/tentar "consertar a fórmula" de novo (2ª vez que isso é tentado nesta pendência, 1ª tentativa documentada piorou o resultado).

**Causa raiz real**: `VARS.cartaoMBTotal` (âncora manual, "a fatura sempre vence") estava calculado como **fatura aberta INTEIRA (16/07-17/08) menos o pagamento** = R$6.480,29 — mas isso mistura o resíduo do ciclo do CARTÃO anterior (fecha dia 22, já pago em 27/07 via TX000234) com o ciclo NOVO. As somas que alimentam "Não Reconciliado" (LRW/LRV via `ciclo_inicio_em`) só contam a partir de **25/07** (ciclo interno do site, Caixa Variável) — comparação era maçã com laranja.

**Usuário confirmou 2 pontos importantes que eu tinha errado**: (1) o ciclo do cartão fecha sempre dia **22**, não no meio da fatura como eu supus a princípio; (2) existe um "limbo" de **22-24/07** desenhado de propósito pra empurrar compras de fronteira pro ciclo seguinte — e a âncora do MB precisa **incluir** esse limbo (não só o corte exato de 25/07), senão a falta de sincronismo entre fatura e ciclo do site volta a atrapalhar.

**Corrigido**: `cartaoMBTotal` recalculado como soma real da fatura **a partir de 22/07/2026** (com o limbo 22-24 incluso) = 87 lançamentos, **R$6.407,98** (era R$6.480,29). Escrito em `parametros_gerais` (fonte real, lida no boot) e no literal de fallback (`vars-mercado-pago.js`), ambos sincronizados.

**2ª causa real achada e parcialmente corrigida, mesmo padrão já documentado antes**: mesmo com a âncora corrigida, uma checagem à parte achou que a soma de Caixa Variável (cartões MB, ciclo atual) no banco (R$3.812,63) não batia com o que LRW/LRV mostravam (R$3.175,63) — 5 transações do cartão consolidado (2250), importadas via Pluggy, chegaram **sem `usuario_id`**, invisíveis pra `vw_transacoes_cartao_variavel_por_pessoa` (mesmo bug já documentado 12/08: "6 lançamentos do Pluggy sem usuario_id sumiram da lista LRW"). Corrigidas 3 das 5 (Digna Vida R$152,41, Pax Domini R$77,79, Amazon BR R$32,06 — `usuario_id` setado pra Wallace). **2 ficaram de fora de propósito**: "Conta Vivo" R$55,08 + R$484,00 (mesmo dia, 27/07) — risco real de dobrar a conta junto com a recorrência "Vivo" já cadastrada (`cronograma_recorrencias`, R$435,08/mês, que tem uma nota própria sobre transição de valor "R$523,00 confirmado pela fatura DESTE ciclo, ainda não aplicado") — não mexido sem entender a relação entre os dois primeiro.

**Achado colateral, não é gap real**: outras transações do mesmo lote tinham `usuario_id` preenchido mas `ja_orcado_assinaturas=true` (Amazon Prime Canais, Netflix, ChatGPT, YouTube Premium, Intelbras Cloud) — excluídas de propósito de LRW/LRV pra não contar 2x junto com assinaturas/recorrências. Comportamento correto, não mexido.

**"Conta Vivo" resolvido**: usuário confirmou — as 2 cobranças (R$484,00+R$55,08=R$539,08) são a MESMA conta Vivo, veio dividida em 2 por causa da transição de plano nesse ciclo específico; a partir de setembro/2026 volta a vir 1 cobrança só. `cronograma_recorrencias` "Vivo" (TXRR000001) atualizado pro valor real deste ciclo (R$539,08, era R$435,08 — futuro volta a R$435,08 a partir de set/2026, nota deixada no `obs`). `usuario_id` das 2 transações brutas (TX000245/246) **não foi setado de propósito** — a recorrência já representa esse gasto em `D.recorrencias`, setar `usuario_id` também contaria 2x.

**Estado ao fim desta investigação**: âncora corrigida (R$6.407,98), 3 transações religadas ao usuário certo, recorrência Vivo atualizada — tudo isso deve reduzir bastante o resíduo mostrado no painel, mas **não testado ao vivo** (sem acesso a login). Próximo passo pro usuário: conferir o card "Não Reconciliado" de novo — se ainda sobrar resíduo relevante, investigar com a mesma técnica (comparar Caixa Variável banco × LRW/LRV item a item, não só o total).
...
**Assinatura Claude atualizada pro valor real** (`cronograma_assinaturas` TXS000002): R$113,72 (estimativa) → **R$120,60** (R$116,52 + R$4,08 IOF, valor real da fatura, cobrado no cartão 8530, data ajustada pra 11/08), seguindo a regra "valor final sempre substitui estimativa" ([[feedback_valor_final_sobre_estimado]]). Pedido explícito do usuário: "use o valor de fatura".

**Duplicata real achada e corrigida — R$551,01 lançados 2x**: `TX000159`/`TX000159-A`/`TX000159-B` (25/07, Mercado Livre, R$196,01+R$319,90+R$35,10, já dividida certo entre Caixa Variável e Bens Duráveis) e `TX000243` (26/07, mesmo valor R$551,01, importada automática via Pluggy, `pendente_classificacao`, nunca categorizada) eram a MESMA compra real, lançada 2x — a fatura real mostra só 1 ocorrência (26/07). Usuário confirmou: "é só 1 vez, use a dividida por caixas" — `TX000243` (a não-categorizada) deletada, mantida a divisão correta nos outros 3. Checagem adicional (mesmo valor ±2 dias, toda a base) não achou nenhuma outra duplicata do mesmo padrão — o único outro par com valor igual é uma cadeia de reembolso legítima (TX000217/218/208, adiantamento + 2 baixas cruzadas, não erro).

**Gap residual final, ~R$35, TODO explicado, nada realmente "sumido"**: depois da duplicata corrigida e da assinatura Claude atualizada, o que sobra são só: (a) pequenas taxas de IOF de compra internacional (~R$16 total, várias linhas de R$0,75-R$4,10) já embutidas nas estimativas em USD dos valores principais, nunca lançadas como linha separada; (b) as 3 recorrências fixas (Faculdade/Brisanet/Rastreador) que não são gap real, só invisíveis pra busca por `cartao_id` (arquitetura conhecida). **Pendência prioritária avançou de verdade nesta sessão — não fechou 100% ainda, mas o resíduo remanescente não representa dinheiro perdido/não rastreado, só diferenças de representação (IOF embutido, Origem fixa de recorrência).** Próximo passo, se quiser fechar de vez: recalcular `mbLRNaoReconciliado` no painel (`hydrate-visa-mb.js`) pra ver se o resíduo exibido melhora depois de todas essas correções na base.
