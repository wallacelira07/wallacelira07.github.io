...
### 1.6 WWI — falta agendar o job mensal no cron-job.org (só o usuário pode fazer) + validar ao vivo
Ver seção 2.13. Módulo completo implementado e migration aplicada em produção, mas 2 coisas ainda pendentes: (1) criar o agendamento mensal (dia 25) no painel do cron-job.org — conta externa, agente não tem acesso; até lá o job só roda via disparo manual (aba Actions do GitHub). (2) Mesma ressalva do relatório de fechamento original (1.3): fluxo completo (botão → iframe logado → busca de histórico → PDF com narrativa/score) não testado com login real, só verificação de sintaxe e testes isolados das funções puras (competência/narrativa Python testados com dado fake).
