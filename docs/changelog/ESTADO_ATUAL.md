...
**Risco residual documentado, não corrigido nesta sessão**: `LRPGV_TRANSACOES` (PIX Geral Vanessa) tem a MESMA constante congelada (`LRPGV_TRANSACOES_CICLO_ATUAL`, app.js linha 1910) e o mesmo padrão de restore (linhas 1938/1946) — mas quem a corrige de verdade é `aplicarOnda3LivroRazao()` (as 13 caixas temáticas do ONDA3_LR_MAPA), não `aplicarOnda3LrwLrv()`. Re-chamar essa função inteira de dentro de `trocarCiclo()` redesenharia as 13 caixas temáticas também — fora do escopo do pedido de hoje (só "compras do limbo"). Fica pendência pra próxima vez que esse sintoma aparecer em PGV.
