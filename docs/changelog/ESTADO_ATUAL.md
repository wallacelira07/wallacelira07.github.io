...
6. **RESOLVIDO NA RAIZ 15/08/2026 — Google Drive sincronizando `.git/`.** Causa raiz eliminada (não mais mitigação reativa): `.git` real movido pra fora da árvore sincronizada (`C:\Users\WLI015\.git-repos\Site.git`), com um arquivo-ponteiro de 1 linha (`gitdir: C:/Users/WLI015/.git-repos/Site.git`) no lugar do diretório antigo dentro do repo — mesmo mecanismo que o Git usa pra submodules/worktrees. `git status`/`git log`/`git pull`/`git push` continuam idênticos, testado ao vivo. **Isso é local à máquina `WLI015`** — se o repo for aberto numa máquina nova sem essa pasta, o Git vai reclamar "not a git repository"; nesse caso, é só recriar o `.git` normal com um `git clone` novo (não é bug, é reconfiguração de uma vez por máquina). Não reabrir como pendência nesta máquina; se `desktop.ini` aparecer de novo dentro de `.git-repos/`, é sinal de que algo moveu esse diretório pra dentro do Drive de novo, não do bug antigo.
...
12. **ATUALIZADO 15/08/2026 — Inbox Financeira DESATIVADA DA UI, pedido explícito do usuário ("desative o inbox, isso só gera duplicidade").** Resumo/tabela/aprovar/rejeitar escondidos (`display:none`, seção 21 de `Sistema_Wallace_Lira_Completo.html`) — não deletado, sincronização Pluggy/Mercado Pago continua rodando por baixo porque outras telas ainda leem `VARS.INBOX_FINANCEIRA`. Não reativar a UI sem pedido explícito novo; causa raiz de "item resolvido reaparecendo" não foi investigada, fica pra quando o usuário quiser retomar essa frente.
