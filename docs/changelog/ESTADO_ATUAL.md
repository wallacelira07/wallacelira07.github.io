...
1. **Código morto / funções legadas — varredura RÁPIDA feita 22/08/2026 (pós-handoff, crédito quase no fim), resultado: nada órfão encontrado, mas superficial.** Checado via grep: (a) nenhuma referência code-level sobrou pro módulo deletado `shadow-homologacao-necessidade.js`/`agendarShadowHomologacaoNecessidade()` — só aparece em docs/comentário histórico, limpo; (b) `REG.operacional.reembolsoSobraPessoal` (mantido em JS de propósito, decisão documentada no bloco 46) é usado em 12 arquivos — não é código morto, decisão confirmada correta. **Isso NÃO é uma varredura completa** — só verificou o que a própria Sub-fase B tocou, não uma auditoria geral do codebase por funções/variáveis não referenciadas em lugar nenhum. Se o usuário quiser uma varredura completa de código morto no projeto inteiro (não só o que esta migração tocou), isso continua pendente.
...
4. ~~Falta documento dedicado de arquitetura final~~ **FEITO 22/08/2026**: [`docs/decisions/ARQUITETURA_NECESSIDADE_RPC_VS_JS.md`](../decisions/ARQUITETURA_NECESSIDADE_RPC_VS_JS.md).
...
- [ ] **PRIORIDADE 0 restante (ver seção no topo)**: teste físico de edição pendente, divergência SSOT de Patrimônio não investigada a fundo. Documento de arquitetura e varredura rápida de código morto já feitos.
