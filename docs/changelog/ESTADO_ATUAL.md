...
**Ciclo Snapshots — só a armazenagem migrada, JS deliberadamente NÃO ligado (bloqueador técnico real, não contornado)**: nova tabela `ciclos_financeiros_snapshots` no Supabase (schema completo: salário, Caixa Variável, reembolso, tolerância, cascata em jsonb, necessidade bruta/líquida, modo operacional, saldo do ciclo, valores congelados de cartão do ciclo fechado, e `livros_razao_arquivados` em jsonb com os 4 arrays de Livro Razão do ciclo fechado). Backfill 100% conferido: ciclo `2026-06` (fechado, 43 lançamentos LRW arquivados) e `2026-07` (aberto, sem arquivo). **Por que o JS não foi religado**: `CICLO_SNAPSHOTS` é lido de forma SÍNCRONA durante a construção inicial do `VARS` (`aplicarCicloAoVARS()`, `recalcularNecessidade()`, `auditoria-automatica.js`, `CycleEngine.js`) — Total Operacional, Necessidade Líquida, Modo Operacional e Saldo do Ciclo dependem desse dado estar pronto antes do primeiro cálculo do boot. Uma busca no Supabase é sempre assíncrona; trocar a fonte exigiria re-executar em cascata toda essa cadeia de cálculo financeiro depois que o fetch resolvesse — risco real de regressão em números que o usuário usa pra decisão financeira diária, sem tempo nesta sessão pra mapear e validar com segurança total. **Não remover o literal de `vars-ciclo-snapshots.js`** até uma sessão dedicada a essa segunda etapa.

**Bugfix fora da fila, reportado ao vivo pelo usuário**: badge de frescor da geração solar disparava alarme falso todas as noites ("atualização antiga, verifique se o robô SAJ está rodando") porque o robô só lê das 6h às 18h (horário de Brasília, mesma janela do cron `*/10 6-18 * * *`) — a pausa noturna é esperada, não uma falha. Corrigido em `formatarFrescor()`/`montarBadgeFrescor()` (app.js, novo parâmetro opcional, comportamento idêntico pra quem não usar) + `agoraEfetivoFrescorSolar()` novo (hydrate-onda5-qualidade-geracao.js) — "congela" a classificação de alarme fora da janela 6h-18h, mas o tempo REAL exibido continua honesto. Validado ao vivo às 21h (dentro da janela noturna real de teste): badge mudou de laranja pra verde, zero erro. Commit `a85b469`.
