...
**(b) O déficit de caixas sem LREI (hoje R$2.458,29 ao vivo) não entrava em NENHUMA projeção de ciclo futuro** — só na Necessidade Bruta do ciclo atual (índice 0). Usuário: *"se eu vou cobrir as caixas negativas com o salário de 25/08, é lógico que esses custos têm que ser somados à necessidade do próximo ciclo"*. Corrigido em `recalcular-necessidade.js`: o loop que gera `REG.evolucao.necessidadeBruta[0..11]` agora soma `REG.operacional.deficitCaixasSemLrei` em TODOS os índices, não só implicitamente no 0 — mesmo tratamento das outras linhas "fixas" (Boletos/Consórcios/Recorrências/Assinaturas): sem dado real de como cada caixa evolui mês a mês, assume-se o valor de hoje constante pra frente; quando a caixa for corrigida de verdade, o próximo recálculo ao vivo já reflete o déficit menor/zerado.

**Efeito combinado**: `REG.evolucao.necessidadeLiquida[1]` (o número que os cards "Necessidade × Salário"/"Estimador"/topo agora mostram) sobe de R$12.025,32 pra **~R$14.483,61** (+ R$2.458,29 do déficit de caixas). Sobra real estimada pro dia 25/08 cai de R$2.493,98 pra **~R$35,69** — muito mais apertado do que o card mostrava antes de incluir o déficit.
