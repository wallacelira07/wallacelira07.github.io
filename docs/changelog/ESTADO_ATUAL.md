...
2. **Gráfico "Cenário Salário" (`cCenarioSalario`, "Ponto de empate")** — desenhado uma única vez, de forma síncrona, no boot; não está na lista `ONDA4_WARTSILA_IDS` que `hydrate-onda4-wartsila.js` resincroniza depois de buscar o dado real da V2. Resultado: mostra `reembolsoSobraPessoal` calculado com os valores ANTIGOS de `reembolso_wartsila_ciclo` (antes da correção desta sessão), não os corrigidos. Com os valores corretos (total R$7.040,61, Wärtsilá R$5.056,95, MP corp R$266,23, cartão corp R$297,31, provMP R$403,11 — confirmado a partir do array real `PARCELAMENTOS_MP`), a sobra pessoal recalculada seria R$1.017,01, não os R$3.090,16 ainda exibidos. Mesma classe de bug já corrigida hoje pro card "Caixa Wärtsilä"/badge `r21Wartsila`, mas esse gráfico específico ficou de fora. Não corrigido ainda — fica registrado.
