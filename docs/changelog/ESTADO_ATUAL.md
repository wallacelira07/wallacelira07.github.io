...
38. **RESOLVIDO bloco 28 — FASE 2F/2G(itens 1-2) não bloqueiam mais por divergência V1×V2.** O gate `aprovado = lote.totalDivergente === 0` virou `aprovado = true` nessas 3 fases (staleness estrutural confirmada — V1 congelado, V2 crescendo com lançamento novo). O Comparator continua rodando, só que agora é diagnóstico (console), não trava mais a promoção. **As outras 14 fases (2G item3/4, 2H-2V exceto 2F/2G1-2) mantêm o gate original de propósito** — são teste de regressão de fórmula (V1×V2 leem as mesmas variáveis, sem fetch novo), não staleness; desligar aquele gate esconderia bug de cálculo real. Não confundir as duas categorias se mexer nesse arquivo de novo.
