...
## 🏁 Encerramento 13/08/2026 (por limite de crédito) — Auditoria 6 abas + reconciliação Wärtsilä + Pluggy investimentos/reserva

Sessão longa, herdou contexto de uma sessão anterior do mesmo dia (13/08). Encerrada não por o trabalho ter acabado, mas por limite de crédito da conta — **ver `ESTADO_ATUAL.md` seção 1 para as pendências exatas de onde retomar**, resumo narrativo aqui.

**Auditoria multi-agente (pedido explícito do usuário — "TUDO, selecione os melhores agentes")**: 2 rodadas de Workflow, 6 agentes cada (1 por aba: Painel/Gráficos/Solar/Cenários/Balanço/Emagrecimento) — primeira rodada auditando estética+legendas, segunda auditando fórmulas+matemática, ambas em modo "achado real, verificado no código, não genérico". ~60 achados. Uma 3ª rodada (aplicação dos achados, cada agente numa worktree git isolada pra evitar conflito de edição simultânea no HTML/CSS compartilhado) bateu no limite de gasto mensal da conta no meio — 5 de 6 agentes falharam, retomado depois com `resumeFromRunId` sem perder o que já tinha sido feito (cache de agentes concluídos preservado). Merge das 6 worktrees: 5 limpas via `git merge --no-ff` (3-way merge automático, zero conflito real apesar do `main` ter avançado bastante desde que as worktrees foram criadas), 1 (Painel) com um estado de working-tree corrompido nunca explicado (git status mostrava TODOS os arquivos como deleted+untracked ao mesmo tempo) — resolvido lendo o conteúdo final da worktree arquivo por arquivo e reaplicando manualmente via Edit no `main`, em vez de arriscar um merge automático sobre um estado incerto.

**Achado de processo real durante a auditoria** (Solar): um dos achados da 1ª rodada dizia que a tabela "Quanto você ainda vai pagar mesmo com 100% dos créditos" estava com "dados insuficientes" sempre por causa de nomes de campo desatualizados. Investiguei com dado ao vivo antes de aplicar a "correção" sugerida e descobri que o achado estava parcialmente errado — a tabela já funcionava (a Supabase tinha campos extras não documentados no fallback local que o agente não viu), o problema real era só a fatura base ainda usar Jul/26 em vez de Ago/26 (já carregada). Conferi a fórmula à mão (bateu exato com os 3 valores renderizados) antes de decidir o que corrigir de fato — lição: **achado de agente de auditoria precisa de verificação contra dado ao vivo antes de aplicar, não só confiar no relatório**.

**Reconciliação real do reembolso Wärtsilä (ciclo 2026-07)** — a parte mais longa e mais corrigida da sessão, várias rodadas de ida-e-volta com o usuário até fechar certo:
1. Usuário trouxe um TED real de R$6.682,76 recebido, pedindo pra fechar a conta da cascata de reembolso — Claude Chat (outra sessão/interface) tinha travado nisso sem conseguir agir. Resolvido calculando a perna 4 (MP pessoal) direto da soma de parcelamentos ativos em vez de esperar o usuário informar (o campo `perna_mp_pessoal_provisionado` é intencionalmente NULL na tabela V2, o valor real vem de outro lugar do sistema).
2. Discrepância de R$17,85 no "quanto falta" — usuário corrigiu o total bruto do ciclo pra R$7.362,76 (não R$7.380,61 gravado). Atualizado na tabela `reembolso_wartsila_ciclo`.
3. **Erro real cometido e corrigido**: marquei a LREI0003 como quitada usando a perna 2 (MP corporativo) deste ciclo, assumindo que o valor idêntico (R$266,23) era coincidência positiva. Usuário corrigiu: são coisas diferentes — a LREI0003 foi criada por uma fatura de um ciclo ANTERIOR (24/07), a perna 2 deste ciclo é uma fatura NOVA (vence 04/09). Revertido (LREI0003 volta pra ATIVO), e depois quitada de verdade usando saldo ocioso disponível (não ligado a nenhum ciclo específico — "é quitado quando o dinheiro for disponível", regra que o próprio usuário formulou).
4. Usuário pediu pra "parar" no meio de uma execução (respeitado imediatamente, nada mais foi registrado até confirmação).
5. **Segundo erro real cometido e corrigido**: rota errada pras pernas 2 e 4 — mandei pra Caixa Lance, usuário corrigiu que cada cartão tem sua própria caixa (perna 2+4 → Caixa "Mercado Pago"; perna 3 → Caixa "Mastercard/Infinite"), exatamente pra evitar a confusão que causou o erro do item 3 (mistura entre coisas de cartões diferentes).
6. 5 transações reais antigas (compras Mastercard Black em viagem, 25-29/07) tinham `caixa_id` apontando pra "Provisionado Wärtsilä" — corrigi pra "Caixa Variável" achando que era erro de dado, usuário apontou que essas 5 alimentam o Livro Razão Corporativo (LRC) exatamente por causa desse padrão `caixa_id=Wärtsilä + cartao_id preenchido` (mecanismo documentado em `getTransacoesCorporativoCartaoDetalhe()`) — revertido de novo.
7. Usuário pediu explicitamente "não registre nada ainda sem eu confirmar que foi feito" depois do 2º erro — regra respeitada no resto da sessão: toda tabela de divisão foi apresentada como PLANO antes de qualquer INSERT/UPDATE real.
8. Fechado: cascata completa registrada (TX000280-302), 3 caixas negativas cobertas como aporte (não empréstimo, distinção que o próprio usuário insistiu ser importante — "o dinheiro da sobra é meu, eu estou quitando"), LREI0005 quitada com saldo próprio da Caixa Boletos, LREI0003 quitada com saldo ocioso, LREI0004 e o R$340 restante ficam pendentes (dependem de dinheiro que ainda não existe).

**Aba Emagrecimento**: seção nova "Aplicações do Ozivy" (semaglutida semanal, 1ª aplicação 0,25mg registrada), meta de peso 110kg adicionada (usuário pesou 139,6kg hoje, faltam 29,6kg), tabela `aplicacoes_ozivy` nova.

**Investigação Pluggy** (pedido do usuário: automatizar saldo de cofrinhos/investimentos que a Pluggy já busca mas nunca persiste em lugar nenhum):
- Confirmado real: `/investments` da Pluggy funciona e traz dado real (testado ao vivo pelo usuário) — mas Mercado Pago e a previdência do BTG especificamente NÃO aparecem nele, é limitação real da fonte, não bug nosso. Persistência nova (`pluggy_investimentos`) capturou com sucesso 3 CDBs reais do Itaú.
- Usuário identificou (dashboard `meu.pluggy.ai`, print real) que esses 3 CDBs SÃO a Reserva de Emergência — card novo no Balanço mostrando saldo real + rendimento a sacar (estratégia dele: sempre R$100k lá, saca o excedente mensal pra Caixa Lance).
- Usuário depois trouxe print do dashboard da Pluggy mostrando "saldo reservado" com cadeado — pesquisei e achei o campo real (`bankData.reservedBalances`, documentado em `docs.pluggy.ai/reference/accounts-list`), implementei a captura + tabela nova (`pluggy_saldos_reservados`) — **usuário rodou o workflow depois do commit e a tabela ficou vazia, não depurado ainda por falta de tempo** (sessão encerrada por limite de crédito logo em seguida). Ver `ESTADO_ATUAL.md` 1.1 para hipóteses de próximo passo.
- BTG será removido pelo usuário (não trazia investimento de valor real) pra abrir espaço pro Pluggy conectar a 2ª conta Mercado Pago nova (onde ele moveu 3 caixas: Bens Duráveis/Emagrecimento/Suavização) — ação dele no dashboard da Pluggy, mais atualização do secret `PLUGGY_ITEM_IDS` quando ele passar o novo item_id (não tenho acesso a Secrets do GitHub nem `gh` CLI neste ambiente).

Todos os commits desta sessão (cronológico): `adec7eb` (Superávit Normal + Ozivy) → `524a83b`/`1c73660` (commit do próprio usuário + merge, capturou os mesmos 2 arquivos que eu ainda não tinha commitado) → `27f00d9`...`46fccb9` (sessão anterior do mesmo dia, salário/cobertura garantida) → `6f62cc9`/`c7d970f`/`be5112d`/`b20e1c7` (fix por aba, auditoria) → `e5f6d4e`/`b0c81f1`/`598bb32`/`d03b9f9` (merges) → `af02cad` (Painel manual + Pluggy investimentos) → `e2c7b25` (meta de peso) → `20ac7a3` (card Reserva de Emergência) → `4fd661d` (saldo reservado, não funcionando ainda). Todos pushed pro `main`.

## 🏁 Encerramento 12/08/2026 (continuação, mesma sessão) — Fase 4: auditoria (performance + componentes) e correção de dado real solar

Usuário definiu escopo da Fase 4: performance adicional + consistência de componentes. Auditoria de código (agente Explore, sem edição) encontrou 10 achados, por prioridade:
1. `src/services/FinanceService.js:157-164` `getResumoPorCaixa()` faz N+1 sequencial (await em loop) em vez de `Promise.all` — hoje sem impacto real porque `DashboardService` que o chama nunca é instanciado em lugar nenhum (código Fase 5 ainda não ligado), mas vale corrigir antes de ativar.
2. `src/app/app.js:90` `invalidarCache(){ this._cache.clear(); }` — qualquer escrita (lançar transação, criar categoria, resolver caixa) descarta o cache inteiro (TTL 90s da Fase 1), forçando refetch de ~20 endpoints mesmo quando só 1 dado mudou. Impacto real hoje.
3. `src/auditoria/inbox/inbox-financeira.js` `renderInboxFinanceira()` reconstrói a tabela inteira (`tbody.innerHTML`) a cada aprovação/rejeição de item da Inbox, em vez de atualizar só a linha afetada. Fluxo usado com frequência, sessão aberta por horas.
4. `Sistema_Wallace_Lira_Completo.html` — vários pontos com "card"/"badge" reimplementados via `style` inline em vez das classes `.card`/`.badge` já existentes (linhas ~1125-1210: blocos do simulador solar, badge com classe E style conflitantes linha 409, badges de pessoa como `<div>` com hex hardcoded em vez de var(--green)/var(--amber)). Popup de lançamento manual (linhas ~2065-2084) é um formulário inteiro fora do design system, cores hardcoded próprias.
5-10. Achados de prioridade menor (botão pill sem classe reutilizável, Chart.js recriado do zero em vez de `.update()`, `new Function` pra carregar módulos). Detalhe completo só na transcrição da sessão, não reproduzido aqui.

**Aplicados 3 dos 10** (usuário escolheu os 3 primeiros da lista de prioridade): (1) `invalidarCache()` trocado de `_cache.clear()` (tudo) pra `_cache.deletarPorPrefixo(lista)` (só as ~18 chaves derivadas de `transacoes`/`caixas`/`patrimonio` — a mesma lista que o comentário de `atualizarPainelAposLancamento()` já documentava como afetada, só passou a ser aplicada de fato; investimentos/indicadores de ações/solar/pluggy/mercadopago/cronogramas fixos/saúde de jobs ficam de fora, não relacionados a um lançamento manual). (2) Inbox Financeira: `inboxAprovar`/`inboxRejeitar` agora chamam `atualizarInboxItem(id)` (substitui só a `<tr>` do item, via `outerHTML`, mais o resumo/badge recalculado) em vez de `renderInboxFinanceira()` inteiro; extraída `renderInboxLinha(it)` do `.map()` pra reaproveitar o mesmo template numa linha isolada; fallback pro render completo se a linha não existir no DOM. (3) Componentes: badge "Benefícios, não patrimônio" trocado de `class="badge bg" style="..."` (conflitante, verde sobrescrito por style amber) pra `class="badge ba"` direto; badges de pessoa (Wallace/Wellida) trocam hex hardcoded antigo (`#34c98a`/`#e8a63a`, já dessincronizado da paleta nova da Fase 3) por `var(--green)`/`var(--amber)`; 11 ocorrências de `border-radius:8px` trocadas pelo token `var(--radius-lg)` (mesmo valor exato, zero mudança visual, só nomeação). Validado visualmente via servidor local (mesmo padrão das fases anteriores) antes de commitar. Commitado (`c8a23db`→rebase→`b7b83c4`) e pushed.

**Continuação (mesma sessão, novo pedido "continue")**: usuário pediu pra aplicar os achados restantes. Mais 2 aplicados: (4) `FinanceService.getResumoPorCaixa()` — `await` sequencial em loop trocado por `Promise.all` (arquivo isolado, sem UI ligada ainda, zero risco). (5) Botão "Compartilhar" (link solar) usava `var(--purple, #9085e9)` — achado extra durante a correção: `--purple` nunca foi definida em `:root`, então sempre caía no fallback hardcoded, que ficou desatualizado desde a Fase 3 (era o `--accent-2` de ANTES do refino de paleta — um bug real de drift que o audit original não tinha citado por nome). Extraído pra `.btn-pill`/`.btn-pill-icon`/`.btn-pill-label` (usando `var(--accent-2)`), reutilizável. Validado visualmente via servidor local antes de commitar. Commitado (`43cb343`→rebase→`c642dcf`) e pushed.

**Deliberadamente NÃO aplicados nesta rodada**: Chart.js recriado do zero em 35 pontos (já protegido contra leak, só não é o mais barato — otimização, não bug; converter pra `.update()` exige checar caso a caso se o formato de dado muda por gráfico, sem como testar visualmente logado). `new Function` pra carregar módulos (mexe no timing/escopo do boot da página, risco de tela branca sem rede de segurança). Ficam de backlog documentado em `ESTADO_ATUAL.md`.

**Continuação (novo pedido "continue", usuário escolheu "aplicar os 3 restantes com cautela")**: dos 3, o formulário de lançamento manual (`#formLancarTx`, `app.js`) foi extraído pra classes CSS reutilizáveis (`.wallace-field`/`.wallace-btn-primary`/`.wallace-btn-secondary`) — MESMAS cores exatas (paleta própria do form, `#0f1620`/`#2d3b52`/`#c8d4e3`/`#5fd68a`, decisão visual antiga e consistente internamente, não um drift acidental), só removeu a repetição de 8+ blocos de `style` inline idênticos. Validado via servidor local reproduzindo o mesmo form isolado, screenshot confirmou layout idêntico. Commitado (`f4c76b7`→rebase→`6de448e`) e pushed. Os outros 2 (Chart.js `.update()`, `new Function`) continuam de fora — expliquei ao usuário que "mais cuidado" não reduz o risco real o suficiente sem conseguir testar no painel logado (são mudanças de comportamento de renderização/boot, não mecânicas como as já aplicadas).

**Interrupção no meio da Fase 4**: usuário mandou 2 faturas reais em PDF (Ago/26, Wellida e Casa da Mãe) pedindo pra atualizar um cálculo de composição tarifária com dados reais (fora do escopo da Fase 4, tratado à parte — ver seção `ESTADO_ATUAL.md` "Correção de dado real — composição tarifária"). Resumo: a tabela "Residual pós-solar estimado" usava % genérico chutado; substituído pelos % reais extraídos da fatura, mais correção estrutural (COSIP é linha separada da fatura, nunca compensada por lei — usar valor real exato, não %). **Achado de processo importante**: a chave `ENERGISA_TARIFA_COMPOSICAO` tem cópia viva no Supabase (`wallace_dados`) que sobrescreve o arquivo local `vars-energia-solar.js` no carregamento — por pouco a correção teria sido feita só localmente e nunca aparecido no site de verdade (exatamente o alerta do `CLAUDE.md`). Corrigido nos 2 lugares. Commitado (`1a0b583`) e pushed.
...
**Fase 2 (mobile/responsividade)**: testadas de verdade, sem login, em tela de 375px, as duas páginas acessíveis publicamente — tela de login (inicialmente pareceu deslocada no screenshot do preview, mas `getBoundingClientRect()`/estilos computados confirmaram que estava corretamente centralizada; o deslocamento era só um artefato de como o painel de preview desenha a emulação mobile) e `solar-compartilhado.html` (testada com token real e ativo do banco — sem overflow horizontal, gráficos e cards com tamanho razoável).

**Bug real corrigido (reportado por print do usuário)**: "os dizeres estouram a tela" na aba LRV (Livro Razão - Vanessa). Causa raiz: o `@media (max-width:640px)` existente em `assets/css/styles.css` (que faz `table{width:max-content}` pra evitar compressão ilegível de colunas) assumia que toda tabela já vivia dentro de um wrapper `overflow-x:auto` — verdade pro Inbox/Opções (que têm esse wrapper manual desde 10/08), mas **não** pros 24 painéis do Livro Razão (`.pane`, seção 15 do CSS: LRW, LRV, LRB, LRP, LRS, LRR, LRCON, LRC, LRMP, LRCV, LREI, LRDOA, LRPGV, LRPV, LRBD, LRCL, LRMN, LRAJ, LREV, LRSF, LRSE, LRCB, LRCH, LRMI). Sem esse wrapper, a tabela era empurrada pra fora da tela sem nenhum jeito de rolar até o resto — exatamente o print do usuário. Corrigido com 1 regra CSS (`.pane.active{overflow-x:auto;-webkit-overflow-scrolling:touch}`), cobrindo os 24 painéis de uma vez por compartilharem a mesma classe. Testado com estrutura isolada real (`.pane`+`table`+`assets/css/styles.css` de produção, criada e removida em seguida) — confirmado numericamente que o painel passa a rolar (`paneConsegueRolar:true`) sem a página em si ganhar scroll lateral (`documentoTemOverflowHorizontal:false`).

**Fase 1 (performance)**: `WallaceFinanceService._cache` (um `Map` puro, sem expiração) trocado por `_CacheComTTL` — mesma API pública (`has/get/set/clear`), TTL de 90s, nenhum dos ~35 métodos que já chamavam `this._cache.*` precisou mudar. Motivação: sessão longa aberta (padrão real de uso deste site) podia mostrar dado desatualizado até F5 manual — causa raiz já confirmada de uma confusão real numa madrugada anterior (Onda 3, LRW/LRV, Comprometido Caixa Variável).

Usuário aprovou explicitamente ("sim") o commit das duas mudanças pendentes. Commitado (`2da024b`) e pushed pro `main`.

**Limite honesto**: o painel principal (`Sistema_Wallace_Lira_Completo.html`) só carrega após login — nenhuma mudança desta sessão foi validada visualmente nele. Próximo passo depende do usuário: testar no celular e reportar o que quebra, ou seguir pra Fase 3 (visual/design) preparando só a proposta de tokens CSS pra aprovação.
...
**Achado que muda o quadro**: a sessão anterior (mesma data, cortada por limite de uso no meio da implementação de backup) já tinha criado a infraestrutura completa — tabela `backups`, funções `criar_backup_completo`/`restaurar_backup`, job `pg_cron` diário — mas nunca testou nada, e `ESTADO_ATUAL.md` não sabia que essa infra existia (ainda registrava backup como "não corrigido"). Investigando antes de assumir que era só burocracia: a implementação estava **quebrada silenciosamente** — a função exigia `auth.role()='service_role'`, mas `pg_cron` roda direto no banco sem JWT (`auth.role()` = `NULL` nesse contexto), então o próprio job agendado nunca teria conseguido gravar um backup sequer. Lição confirmada de novo: verificar estado real > confiar em documentação ou em trabalho aparentemente "pronto".

**Corrigido e testado nesta sessão**: check trocado pra bloquear só `anon`/`authenticated` (deixando passar contexto direto de banco); `REVOKE EXECUTE` de `anon`/`authenticated` nas 2 funções (proteção estrutural, fechou 4 `WARN` do advisor); backup real executado com sucesso (42 tabelas, 328 transações, bate exato com a contagem ao vivo); restore validado sem tocar produção (roundtrip em tabela `TEMP`, contagem e soma de valores idênticas — não executei `restaurar_backup` de verdade contra produção por ser destrutivo e o plano free não ter staging). Comparação objetiva feita contra upgrade pago (Pro+PITR = US$125/mês mínimo, confirmado por busca web) — decisão registrada: manter a solução própria gratuita. Documentado em `docs/decisions/BACKUP_RESTORE.md` (procedimento de restore passo a passo, RTO~minutos, RPO~24h, riscos residuais honestos: restore completo nunca rodou ao vivo, backup mora dentro da mesma instância).
...
**1. PENDENTE, não resolvido — "Patrimônio Total" aparece como "—" no card da capa.** Adicionei `coverPatrimonioTotal` (novo id) em `Sistema_Wallace_Lira_Completo.html` (linha ~188) lendo `R.balanco.patrimonioTotalGeral` em `hydrate-resumo-p2p.js`. Print do usuário mostrou o valor vazio ("—"), enquanto "Líquido" (o campo antigo, `coverPatrimonio`) apareceu certo. Investiguei a ordem de execução: `recalcularPatrimonio()` (app.js:1339, escreve `REG.balanco.patrimonioTotalGeral`) roda ANTES de `hydrateResumoP2P()` (app.js:1514) na mesma cadeia síncrona — pela leitura do código, deveria funcionar. `REG.balanco.pgbl`/`.fgts` (usados na fórmula) são inicializados de `VARS.patPgbl`/`patFgts` em `reg-balanco.js`, síncrono, carregado antes do app.js. **Não confirmei se é só cache do navegador** (mesmo padrão do bug do `__V` desta sessão, ver item 3) **ou um bug real** — não tive tempo de testar em navegador de verdade. Primeira coisa a fazer na próxima sessão: pedir pro usuário dar F5 puro (não só recarregar a aba) depois de conferir que `__V` bate com o commit mais recente; se persistir, abrir console e checar `REG.balanco.patrimonioTotalGeral` e `REG.balanco.pgbl`/`.fgts` diretamente.

**2. PENDENTE — abas cortando a parte de cima ao clicar.** Usuário reportou 2 sintomas relacionados de scroll no mobile: (a) as abas (Painel/Gráficos/Solar/Cenários/Balanço, `.master-tabs`, `position:sticky;top:0`) "descem um pouco e travam" ao rolar, obrigando a rolar tudo de volta pro topo pra trocar de aba; (b) ao clicar numa aba, o conteúdo aparece cortado por cima (a página não rola até o topo da nova aba). Não achei a causa raiz lendo o CSS (sticky parece corretamente configurado) — pedi print/gravação ao usuário 2x, ele não respondeu ainda. Não mexer sem evidência visual real, só ler código não foi suficiente pra achar isso.

**3. Achado real, corrigido: bug de cache manual (`__V`).** O site usa `var __V = 'AAAAMMDD-N'` como cache-buster manual em `Sistema_Wallace_Lira_Completo.html` — **qualquer edição em `.js`/`.html` só aparece pro usuário depois de bumped essa string**. Esqueci disso na primeira leva de edições desta sessão, o que causou confusão real (usuário testou e nada tinha mudado). Bumped 3x ao longo da sessão, terminou em `'20260810-15'`. **Sempre bumped `__V` a cada leva de edição antes de pedir pro usuário testar.**

**4. Bug real de dado corrigido: gráfico "Necessidade Líquida" não batia com o card do mesmo valor.** Duas causas raiz empilhadas, ambas corrigidas:
   - `ANCHOR_MONTH_CICLO` (`graficos-utilitarios.js`) estava gravado com o mês CALENDÁRIO em vez do mês de CICLO usado por `ciclosDesdeAncoraCiclo()` — descartava o ponto "hoje" do gráfico desde o dia em que foi definido. Corrigido pra `'2026-08'`.
   - Achado mais sério: `atualizarGraficosNecessidade()` (que resincroniza os gráficos depois de um recálculo assíncrono, ex: déficit de caixas sem LREI) mora só no módulo *lazy* `graficos-cenarios-lazy.js` — se o usuário nunca abriu a aba Gráficos/Cenários, a função não existe e os 2 gráficos do Painel principal (sempre visíveis) ficam congelados no valor do boot. Criei versões não-lazy (`atualizarGraficosPainelPrincipal`, `atualizarGraficoPainelPatrimonio`, `atualizarGraficoPainelCaixaVariavel` em `graficos-painel-principal.js`) e conectei nos mesmos pontos de chamada. **Confirmado funcionando pelo usuário** (print mostrando R$13.700 no card batendo com o gráfico).
   - **Suspeita não confirmada**: outros gráficos podem ter o mesmo padrão de bug (função de update só no lazy). Só corrigi os 3 que achei nesta varredura (Necessidade Líquida, Patrimônio, Caixa Variável) — não fiz varredura exaustiva de todos os ~26 `new Chart(...)` do site.

**5. Legendas — 12 chaves revisadas, sem data/histórico de correção**, nos 2 lugares (fallback local `vars-operacional.js` + tabela `legendas` no Supabase, que sempre vence): `legEscolaJulioForaPatrimonio`, `legVisaAposentado`, `legVisaCorrecaoV207`, `legAguaGasMedintech`, `legMigracaoAssinaturasMB`, `legPGVSaldoResidual`, `legOpcoesReconstruido`, `legLinha4vs5MP` (+ frase nova pedida pelo usuário sobre Wärtsilä/corporativo), `legSimulacaoMesAMes`, `legTotalOperacionalDefinicao`, `legTaxasPorHoraAviso`, `legQgHojeParcial` (corrigida 2x — primeiro tirei a data, depois o usuário apontou que a frequência real é 10 min, não uma captura única). Também migrei `legPGBLDefinicao` de HTML hardcoded pro sistema de legendas (não existia como legenda de verdade antes). **Não fiz varredura exaustiva de todas as legendas do site** — só as que apareceram nos exemplos do usuário + as que achei com o mesmo padrão de data/histórico dentro do objeto `VARS.LEGENDAS`.

**6. Redesenho de cabeçalho + mobile (`index.html`)**: nome abreviado "Wallace Lira" (era "SWL"), avatar vira círculo com a inicial (tooltip de clique/toque mostra o email, não só hover), botões do cabeçalho agrupados num bloco único (`.header-actions-group`) pra alinhamento consistente, busca com destaque de cor, ícones proporcionais no mobile, lupa de busca que ficava torta no overlay mobile corrigida. **Confirmado funcionando pelo usuário** (prints).

**7. "Verificações de Negócio" — corrigido 2x na mesma sessão.** 1ª tentativa: movida pra área "home" (capa, fora de qualquer aba) — **errado**, isso é compartilhado por TODAS as abas, vazou pra Gráficos/Solar/Cenários/Balanço (usuário reportou com print). 2ª tentativa, corrigida: movida pra dentro do próprio `#painel` (só existe quando essa aba está ativa), como primeiro elemento. Confirmar com o usuário que não vaza mais depois do deploy.

**8. Tabelas quebrando texto letra por letra no mobile** (`assets/css/styles.css`) — causa raiz: `table{width:100%;table-layout:fixed}` forçava toda tabela (Inbox Financeira, Opções vendidas, etc — até 8 colunas) a caber exatamente na tela. Corrigido com media query `max-width:640px` trocando pra `table-layout:auto` + scroll horizontal (que já existia via `overflow-x:auto` nos wrappers, mas nunca disparava). **Não testado em navegador real** — só raciocínio de CSS, confirmar com o usuário.

**9. Botão flutuante "＋ Lançar" removido, migrado pra dentro da Inbox Financeira** (pedido explícito do usuário — "não combina com nada, atrapalha mais que ajuda"). Era um dock `position:fixed` com comportamento especial de "aba deslizante" no mobile — tudo isso removido, o formulário (grava em `transacoes` via RPC `lancar_transacao_manual`) agora é um bloco inline normal dentro do card Inbox Financeira (`#lancarTxSlot`). Mesma lógica/RPC internas, só mudou onde vive na página. **Não testado em navegador real** (não consigo logar).

**10. Card "Patrimônio" na capa — renomeado e expandido.** "Patrimônio" (ambíguo) virou "Patrimônio Líquido" em 3 lugares (capa, KPI do Painel, Resumo Executivo) pra deixar claro que é só Reserva+BTG/Necton+Caixa Lance+Necton CC. Adicionei "Patrimônio Total" (Físico+Financeiro+Previdência+FGTS líquido de passivos, `REG.balanco.patrimonioTotalGeral` — já existia só na aba Balanço) como valor principal do mesmo card, com Líquido como linha secundária pequena. **Ver item 1 — o valor Total não está aparecendo, prioridade máxima pra próxima sessão.**

**11. Proposta aprovada, NÃO implementada ainda: compartilhamento público (só leitura) da aba Energia Solar.** Usuário quer mandar um link pra família ver só os dados solares, sem dar acesso ao resto do sistema financeiro nem exigir login. Desenho aprovado pelo usuário ("concordo"): token aleatório com validade em dias (RPC pra gerar), página HTML nova sem login (`solar-compartilhado.html` ou nome similar), RPC própria que valida o token internamente antes de devolver só dados de solar (nunca expor tabelas financeiras via esse caminho), botão "Desativar" pra revogar antes do prazo. **Nada disso foi construído ainda** — é a maior peça de trabalho pendente desta sessão. Precisa: 1 tabela nova no Supabase (token/validade/aba/ativo), 2 RPCs (criar link / consultar com token), 1 página HTML pública nova, botão "Compartilhar" na aba Solar.

**12. Cartões (Mastercard Black/Visa/Mercado Pago) — decisão de arquitetura proposta, aprovada em conceito, NÃO implementada.** Descoberta real: o Pluggy já traz o valor REAL da fatura direto do banco (`conta.fatura_mes_atual.valor_total`, `pluggy-reconciliacao.js`), hoje usado só pra comparar contra o valor manual (`cartaoMBTotal`/`cartaoInfiniteTotal`/`mercadoPagoFatura`) e apontar divergência — nunca vira a fonte de verdade da tela. Usuário concordou em trocar pra usar o valor da Pluggy como fonte principal (resolve IOF/juros automaticamente, sem depender de reconciliação manual, sem abandonar o princípio "fatura sempre vence" já que o valor vem direto do banco). **Não implementado** — próxima sessão pode simplesmente trocar a leitura de `VARS.cartaoMBTotal` (manual) pra `resultado.mercadoPagoSaldoAbertoPluggy`/equivalente por cartão, com fallback pro valor manual se a Pluggy estiver fora do ar (mesmo padrão `marcarIndisponivelV2` já usado em outros lugares). **Isso formalmente reabre/substitui a exceção arquitetural registrada em `docs/decisions/EXCECAO_ARQUITETURAL_HEADLINE_TOTALS_CARTOES.md`** — atualizar esse documento quando implementar, não deixar desatualizado.

**13. Filtro de assinatura/recorrência conhecida na Inbox — aprovado, NÃO implementado.** Usuário reclamou da Inbox "bagunçada" (compra toda hora, coisas já reportadas, muita assinatura/recorrência). Causa raiz identificada (não 100% confirmada): dedup hoje só compara VALOR EXATO (`getValoresConhecidosV2()`/`getValoresCombinadosV2()`, `pluggy-reconciliacao.js`) — se uma assinatura já confirmada muda de valor (reajuste, câmbio), a próxima cobrança não bate com nenhum valor conhecido e reaparece na Inbox como gasto novo, todo ciclo, pra sempre. Usuário concordou com a proposta: comparar também por descrição/nome do estabelecimento contra assinaturas/recorrências já confirmadas. **Não implementado.**

**14. `cartao_id` faltando em 21 transações "Assinaturas"** — investigação Nível A feita (query real no Supabase), proposta de classificação por data de corte (Visa aposentado 16/07/2026: antes=Visa 4844, depois=MB consolidada 2250) apresentada, mas **usuário não confirmou execução ainda** — 1 transação (`TXS000003`) pode ser duplicata de `TXS000008`, precisa decisão explícita antes de tocar em qualquer uma (cancelar vs só classificar).

**15. Rodapé "ERP V11"** — identificado como nome morto (não reflete mais o sistema, que roda 100% Supabase/V2). Proposta de troca feita, **usuário nunca confirmou "sim"** — não mudei. Perguntou de novo nesta sessão "isso não já deveria ter sido corrigido?" — esclarecer que a proposta nunca foi aprovada, não que eu esqueci.

**16. Tudo commitado e pushed** (`wallacelira07/wallacelira07.github.io`, branch `main`) em 2 levas: `7410a9d` (fix do gráfico Necessidade Líquida + legendas + Verificações de Negócio 1ª tentativa) e `e15131c` (cabeçalho/mobile/Lançar/Patrimônio Total). Usuário autorizou os 2 pushes explicitamente antes de cada um.

**Regra confirmada nesta sessão pelo usuário, registrar como permanente**: ao corrigir algo já pedido antes, não narrar de volta "você pediu X antes" — agir direto, a documentação do projeto é que serve pra isso, não precisa lembrar o usuário do que ele mesmo pediu.
