...
**Deliberadamente NÃO aplicados** (risco > benefício sem conseguir testar logado): formulário de lançamento manual (`#formLancarTx`) tem paleta própria consistente (não é drift acidental — decisão visual antiga separada dos tokens principais), converter pra `.form-control` mexeria num fluxo crítico sem rede de segurança visual. Chart.js recriado do zero em 35 pontos (já protegido contra leak, só não é o mais barato — otimização, não bug). `new Function` pra carregar módulos (arquitetural). Ficam de backlog documentado em `ESTADO_ATUAL.md`.
