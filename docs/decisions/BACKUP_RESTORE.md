...
2. ~~Backup vive dentro do mesmo projeto/instância~~ — **resolvido na sessão seguinte**, mesmo dia. Ver `docs/decisions/CONTINUIDADE_NEGOCIO_DR.md`: cópia externa (dados criptografados + schema via `pg_dump`) implementada dentro do repositório GitHub, infraestrutura independente da Supabase.
