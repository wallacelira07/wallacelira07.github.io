...
**Exceção — 2 caixas NUNCA são destino de uma compra nova, mesmo sendo tecnicamente "uma caixa" (esclarecido pelo usuário 22/08/2026)**: `Caixa Wartsila` e `Caixa Mercado Pago` são **pagadoras da fatura de um cartão específico** (mecanismo deliberado que alimenta o LRC/reconciliação daquele cartão — ver seção 1.3.5 sobre "caixa_id='Provisionado Wärtsilä' DE PROPÓSITO"), não caixas de orçamento genéricas. Oferecer uma delas como opção pra uma compra qualquer confunde: a primeira com a cascata de reembolso Wärtsilä, a segunda com "quem paga a própria fatura do cartão Mercado Pago". Se o usuário perguntar "quais caixas posso usar pra uma compra nova no cartão", excluir essas 2 da lista.
