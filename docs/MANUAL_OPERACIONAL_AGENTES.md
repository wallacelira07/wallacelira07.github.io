...
**Exceção formal, CONGELADA — não é pendência ativa, não reabrir sem pista nova** (esclarecido 15/08/2026, achado de ambiguidade da auditoria de 43 especialistas: a redação anterior parecia uma pendência em aberto quando na verdade é uma exceção formal já aceita, mesma régua da Migração V1→V2 "formalmente encerrada"): 4 caixas (Caixa Lance + Manutenção + Saúde Família + Aniversário Júlio) continuam exibindo o valor V1 — divergência de R$4,37 (Lance) e R$107-346 (as outras 3) sem causa raiz confirmada, documentada em `docs/decisions/EXCECOES_FORMAIS_DESLIGAMENTO_V1.md`. Lançar nelas grava normal na V2; o número na tela só volta a se mover se o usuário trouxer uma pista nova concreta pra fechar a causa raiz — não é trabalho pendente de nenhum agente.
