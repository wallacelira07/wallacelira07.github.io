...
// (não o Tool Runner beta) para manter este protótipo sem dependência beta e
...
/**
 * Processa uma mensagem de Telegram e devolve o texto de resposta.
 * @param {object} params
 * @param {number} params.chatId - chat_id do Telegram de quem escreveu.
 * @param {string} params.texto - texto da mensagem.
 * @returns {Promise<string>} resposta a enviar de volta pelo Telegram.
 */
async function processarMensagem({ chatId, texto }) {
  const mensagens = [{ role: 'user', content: texto }];
  const contexto = { chatId };
