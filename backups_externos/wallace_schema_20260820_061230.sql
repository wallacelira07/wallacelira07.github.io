...
\restrict qcna5M3iQmBcM7nfSZLUNSaecAXFE8R9WfGX8lQSolTjtv8CSi1WVAIZmKMY36R
...
BEGIN
  ...
  -- NOVO 19/08/2026 (pedido do usuario): consumo real da fatura Energisa (energia_solar_consumo_referencia,
  -- automatizado via scripts/sync/atualizar_boletos_medintech.py), exposto no compartilhado igual ao painel
  -- privado (que ja le direto de window.WALLACE_SOLAR_CONSUMO_REFERENCIA_V2). So wallace/irma (Wellida) -
  -- mae fica de fora de proposito, esta pagina eh so pro Wallace/Wellida.
  SELECT jsonb_agg(jsonb_build_object(
...
BEGIN
  ...
  -- Data civil de Brasilia no instante real da leitura (fuso America/Sao_Paulo - Brasil nao usa
  -- horario de verao desde 2019, resolve sempre -03:00). Esta e' a UNICA fonte da coluna `data`
  -- desta tabela a partir de agora - nunca CURRENT_DATE, nunca a data "de hoje" do agente.
...
COMMENT ON COLUMN public.cronograma_recorrencias.ultima_cobranca_em IS 'Data real da última cobrança confirmada (fatura/extrato) — usada pra saber se essa recorrência já foi cobrada dentro do ciclo atual do cartão (fecha dia 22) antes de contar no card Não Reconciliado. NULL = nunca confirmada, tratada como "não cobrou este ciclo" por segurança.';
...
COMMENT ON COLUMN public.energia_solar_leituras.geracao_acumulada_atualizado_em IS 'Timestamp real de quando o robo SAJ (atualizar_geracao_saj.py) gravou geracao_acumulada nesta linha. Usado pra trava de descompasso (LIMITE_DIAS_DESCOMPASSO_SEGURO) no painel privado e na pagina compartilhada - achado de auditoria 18/08/2026: campo nunca existiu, trava nunca disparava na pratica.';
...
COMMENT ON TABLE public.medidor_tuya_consumo_diario IS 'Consumo real do apartamento (Wallace) por dia civil de Brasília, calculado e mantido automaticamente por trg_medidor_tuya_consumo_diario a cada nova leitura em medidor_tuya_leituras. "Hoje" cresce ao vivo durante o dia; dias passados ficam congelados assim que a 1a leitura do dia seguinte chega.';
...
COMMENT ON COLUMN public.medidor_tuya_consumo_diario.casa IS 'Mesma coluna casa de medidor_tuya_leituras, propagada pelo trigger trg_medidor_tuya_consumo_diario.';
...
COMMENT ON TABLE public.medidor_tuya_leituras IS 'Leituras do medidor de energia EKAZA (CT 80A) via Tuya Cloud API — 1 linha por execução do robô atualizar_medidor_tuya.py. Ver docs/decisions/INTEGRACAO_MEDIDOR_SMART_LIFE_TUYA.md.';
...
COMMENT ON COLUMN public.medidor_tuya_leituras.casa IS 'Qual casa/medidor esta leitura pertence: wallace (apartamento, em producao), wellida (irma, aguardando instalacao fisica), outras conforme runbook COMO_CONFIGURAR_NOVO_MEDIDOR_TUYA.md.';
...
CREATE VIEW public.vw_transacoes_cartao_variavel_por_pessoa AS
 SELECT u.nome AS usuario_nome,
    u.id AS usuario_id,
    t.tx_legado,
    t.data,
...
--
-- Name: medidor_tuya_consumo_diario medidor_tuya_consumo_diario_data_casa_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.medidor_tuya_consumo_diario
    ADD CONSTRAINT medidor_tuya_consumo_diario_data_casa_key UNIQUE (data, casa);
...
\unrestrict qcna5M3iQmBcM7nfSZLUNSaecAXFE8R9WfGX8lQSolTjtv8CSi1WVAIZmKMY36R
